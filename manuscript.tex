...
%
Certain complex stochastic processes can be efficiently modeled with
the discrete--event (hereafter, DE) simulation approach.
%
...
%
We follow this approach to simulate the statistical properties of a
set of intelligent civilizations (CETIs) in the Galaxy that have the
ability of emitting and communicating messages at light speed.
%
Since this method to explore several models easily, it is well suited
for the particular case of the difusion of intelligence signals in the
Galaxy and its dependence on the statistical properties proposed for
the rise and fall of the communicating capacity in the galactic
...
the distribution of lifespans and the distribution of the number of
CETIs per unit time \citep{maccone_evolution_2014,
Sotos_biotechnology_2019}.
%
The shapes of these distributions are discussed in
Sec.~\ref{SS_PDF_shape}.
%
...
as the squared distance from the source, imposes a detectability
limit.
%
...
communicating capacity, each CETI will fill a spherical shell region
of the Galaxy, limited by two surfaces.
%
...
CETI, even if it has disapeared before the time of contact.
%
...
until the surface of last contact reaches $D_{max}$, and producing as
a result the loss of all signal from the central CETI.
%
In our approach, we consider a model Galaxy where the width of the
disk is negligigle with respect to the radius of the disk, so that
...
that result from the existence and communications of two CETIs (panel (b)).
%
The points in time where the CETIs aquire their communicating
capacity (CC) are dubbed ''awakening'' events ($A_1$ and $A_2$ in the
Figure).
%
Simmilarly, the points in time where the CETIs loss their communicating
capacity are dubbed ''doomsday'' events ($D_1$ and $D_2$ in the
Figure).
%
...
in the Figure.
%
It is important to notice that there is a time delay between the contact
events of the two CETIs involved in this analysis, and also a time
delay between the two blackout events, so that the time range when a
bidirectional contact is possible occurs between the maximum time of
the contact points and the minimum time between the blackout points.
